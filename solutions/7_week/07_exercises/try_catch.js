...
function task2() {
  try {
    console.log(ruth);
      ...
      if (error instanceof ReferenceError) { // to specify error
...
function task3() {
  try {
    throw new Error('Something is wrong with the line!');
  } catch (error) {
    console.log ('Caught an error:',error.message )  // can also be console.error: it highlights the background to red
  }finally{
...
function task4() {
  try {
    ...
    try {
      throw new Error ('This is from the inner try-catch')
    } catch (error) {
      console.error ('caught an inner error:' , error.message)
    }

    throw new Error ('This is from the outer try-catch')
  } catch (error) {
    console.error('caught an outer error:', error.message)
  }finally{
...
function task4() {
  try {
    ...
    try {
      throw new Error ('This is from the inner try-catch')
    } catch (inner_error) {   // inner-error can be renamed and reference on the next line eg: inner_error.message
      console.error ('Caught an inner error:' , inner_error.message)
    }

    throw new Error ('This is from the outer try-catch')
  } catch (outer_error) {
    console.error('Caught an outer error:', outer_error.message)
  }finally{
    console.log('Operation attempt finished')
  }
}
...
function task5(array, index) {
 try {
   if(index < 0 || index >= array.length){
    throw new RangeError ('Index out of array length')
   }
   return array[index];
...
task6({firstname:'Ruth'}, 'firstname')
...
Also JSON always has the "-" with the properties
...
function task7(jsonString) {
  ...
  try {
    ...
    console.error ('Invalid JSON format')    // continue from here
